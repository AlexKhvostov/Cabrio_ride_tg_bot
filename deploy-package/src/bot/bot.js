...
bot.on('new_chat_members', async (msg) => {
    try {
        ...
        for (const newMember of new_chat_members) {
            ...
            console.log('👤 Приветствуем нового участника:', newMember.first_name);
...
async function showUserCars(msg, userId) {
    try {
        ...
        let carsText = `🚗 *Ваши автомобили (${cars.length})*\n\n`;
        
        cars.forEach((car, index) => {
            carsText += `**${index + 1}.** ${car.brand} ${car.model}`;
            if (car.generation) carsText += ` (${car.generation})`;
            carsText += `\n📅 Год: ${car.year}`;
            if (car.reg_number) carsText += `\n🔢 Номер: ${car.reg_number}`;
            if (car.color) carsText += `\n🎨 Цвет: ${car.color}`;
            carsText += `\n📊 Статус: ${car.status}`;
            if (car.description) carsText += `\n💭 ${car.description}`;
            carsText += `\n\n`;
        });
        
        bot.sendMessage(msg.chat.id, carsText, { parse_mode: 'Markdown' });
...
async function showStats(msg) {
    try {
        ...
        const statsText = `📊 *Статистика клуба*\n\n` +
            `👥 Всего участников: **${stats.totalMembers}**\n` +
...
async function completeAddCar(msg, userId, data) {
    try {
        ...
        if (isFirstCar) {
            ...
            // Отправляем фото автомобиля если есть
            if (data.photos && data.photos.length > 0) {
                await sendGroupPhoto(data.photos[0], carMessage);
            } else {
